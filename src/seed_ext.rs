...
use crate::{
    ...
    css::Style,
    events::Events,
    prelude::{El, ElRef, Node, UpdateEl},
};
...
// TODO: add try_add(), try_set() functions
pub trait ElExt<Msg> {
    // fn add<'a>(self, val: impl Into<AddArgument<'a, Msg>>) -> Self;
    // fn try_add<'a>(self, val: Option<impl Into<AddArgument<'a, Msg>>>) -> Self;
    // fn set<'a>(self, val: impl Into<SetArgument<'a, Msg>>) -> Self;
    // fn try_set<'a>(self, val: Option<impl Into<SetArgument<'a, Msg>>>) -> Self;
...
impl<Msg> ElExt<Msg> for El<Msg> {
    ...
    //         SetArgument::Attribute(val) => self.set_attribute(val),
